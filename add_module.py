...
def create_new_node(module_name, module_info, existing_nodes):
    ...
    for input_type in module_info.get("inputs", []):
        ...
        inputs.append({
            "Id": port_id,
            "DataType": DATA_TYPE_MAP.get(input_type, 0), # 使用映射，未知类型默认为0
    ...
    new_node = {
        ...
        "ModelVersion": 1,
        "Version": "0.1",
        "OperationType": OPERATION_TYPE_MAP[module_name],
        "Inputs": inputs,
        "Outputs": outputs,
        "VisualPosition": new_position,
        "VisualCollapsed": False,
        "MechanicConnectionId": None,
        "GateDataType": DATA_TYPE_MAP.get(module_info.get("outputs", ["Number"])[0], 2), # 默认使用第一个输出的类型
        "SaveData": None
    }
...
def main():
    ...
    try:
        ...
        print(f"错误: 找不到文件 {e.filename}。请确保 data.json 和 allmod.json 文件与脚本在同一目录下。")
        return
    ...
    # indent=2 使其更具可读性，如果游戏要求紧凑格式，则可设为 None
